-- ============================================================================
-- DDS — 0167_alert_logs_search_unspecified_driver
-- ----------------------------------------------------------------------------
-- Reported bug: Analytics' Spike Investigation panel shows a "Unspecified"
-- driver row (e.g. 2,065+ live Sleep Alert rows in one recent window) with a
-- "View in Alert Logs" button — clicking it lands on Alert Logs with that
-- row's count nowhere to be found.
--
-- Root cause, confirmed live: Spike Investigation (0157) buckets by
-- dds_case_records.emp_no_norm, which COALESCEs a null emp_no to the literal
-- sentinel string 'UNSPECIFIED' (0094). Its "View in Alert Logs" button
-- passes that same id straight through as free-text search
-- (window.__ddsOpenAlertLogs({ search: 'UNSPECIFIED' })), but
-- dds_alert_logs()'s p_search only ILIKE-matches RAW fields — asset_id,
-- operator, event_code, case_driver_name, case_emp_name, emp_name,
-- case_emp_no, emp_no — never emp_no_norm. None of those raw fields
-- literally contain the text "UNSPECIFIED" for an unresolved-driver row
-- (emp_no is genuinely null, operator is genuinely blank), so the search
-- always returns zero rows despite the underlying events being real and
-- present. Confirmed live: querying dds_case_records directly finds 2,065
-- Sleep Alert rows with emp_no_norm = 'UNSPECIFIED', while the exact same
-- WHERE-clause p_search logic dds_alert_logs() runs today matches 0 of
-- them.
--
-- Fix: add emp_no_norm to the same ILIKE OR-chain, the same way every other
-- field there is matched. This makes the existing cross-page bridge work
-- (the id it already sends is exactly this sentinel), and also lets anyone
-- type "unspecified" into the Alert Logs search box directly and get real
-- results, rather than only fixing the one broken caller.
--
-- No signature change, no edit-lock guard needed (STABLE/read-only, not
-- SECURITY DEFINER) — same shape as every other read RPC change in this
-- app's history. Body pulled from this database's CURRENT live definition
-- via pg_get_functiondef() immediately before writing this migration,
-- confirmed identical to 0156's.
-- ============================================================================
