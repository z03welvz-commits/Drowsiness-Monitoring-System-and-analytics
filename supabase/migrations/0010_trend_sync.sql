...
-- DDS — 0010_trend_sync
-- ----------------------------------------------------------------------------
-- Adds a per-day average sync delay to the `trend` array, so the client's
-- Overview Trend card can plot Alert Volume / Distinct Assets / Avg. Sync
-- Delay as 3 independent small-multiple sparklines (see charts.trendMini()
-- and derive()'s matching byDate.syncSum/syncN accumulator in index.html) —
-- both the local-import and signed-in paths must return the same trend
-- shape, and this is the signed-in half of that pairing.
--
-- Only additive: every field dds_metrics() already returned is unchanged
-- (trend gains one new key, avgSyncSeconds; nothing else moves), so this is
-- safe to apply without touching any existing chart/KPI consumer. As with
-- 0006 before it, this migration should be checked against the LIVE function
-- definition (pg_get_functiondef) before applying, in case of further
-- undocumented drift beyond 0006's own on-disk body — 0006 is the base this
-- migration is written against, since no migration between it and this one
-- (0007-0009) touches dds_metrics().
-- ============================================================================
...
begin
  ...
  -- avg_sync_seconds added here (was units/events/assets only) — same
  -- null-when-unmeasured semantics as kpis.avg_sync_seconds above: a day
  -- with no synced rows yields sql null, not 0, so the client trend line
  -- doesn't falsely dip to zero (see derive()'s matching JS-side comment).
  trend as (
    select shift_date,
