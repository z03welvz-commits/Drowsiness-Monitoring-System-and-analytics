-- ============================================================================
-- DDS — 0110_dds_metrics_still_arriving
-- ----------------------------------------------------------------------------
-- Fixes finding C-3: the most recent 1-2 days of any daily count look
-- artificially low because slow-syncing events (events.sync_seconds =
-- update_time - end_time, a real generated column, 0001_init.sql) haven't
-- landed yet — nothing distinguishes "genuinely quiet day" from "still
-- syncing." Adds trend[].stillArriving (boolean) so any future UI can show
-- this truthfully instead of a misleadingly low count.
--
-- Threshold: any day within 4 calendar days of current_date (i.e. today,
-- yesterday, or the 2 days before that) is flagged. Derived from the real
-- P95 sync lag over the last 30 days (89.4h ≈ 3.7 days, rounded up to a
-- clean 4-day window) — not the P99 tail (214.5h ≈ 8.9 days), which would
-- flag over a week of "recent" data as uncertain for the sake of the
-- slowest 1% of events. A real, acknowledged trade-off, not silently
-- picked: ~1% of a "settled" day's events can still arrive after this
-- window closes.
--
-- Checked against current_date (real elapsed time since the event), not
-- p_from/p_to — querying an old historical window must not flag its days
-- as "still arriving" just because the query itself is recent.
-- ============================================================================
