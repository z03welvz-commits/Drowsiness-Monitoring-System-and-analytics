-- ============================================================================
-- DDS — 0117_required_attention
-- ----------------------------------------------------------------------------
-- New RPC backing Overview's "Required Attention" table. Driver-only (the
-- table's own header row has no asset column), and deliberately a SEPARATE
-- function from dds_corrective_actions_queue() rather than an extension of
-- it: that function's "required" status is pinned at "streak >= 3 days OR
-- latest day > 10" (0088/0114, shipped per explicit instruction) and several
-- other places — Corrective Actions itself, dds_entity_status_reopen() —
-- depend on that exact 3-day bar. This RPC needs a different, additive
-- severity split (High vs Critical) that fires at streak >= 2, so it computes
-- its own population instead of quietly lowering that shared threshold.
--
-- Flagging rule (per explicit instruction):
--   - A driver is flagged when their most recent active day's sum(event_count)
--     is > 10. This single condition already covers a lone day at 20+ (a day
--     that qualifies for the streak below is also, trivially, > 10).
--   - Severity is 'critical' when that day is the tail of a run of 2 or more
--     CONSECUTIVE days each >= 20 (the existing streak-day threshold every
--     other page uses); otherwise 'high'.
--   - Excluded once entity_status resolves to 'actioned' or 'resolved' (the
--     "done"/"complete" cases) — same derivation dds_driver_streaks() and
--     dds_corrective_actions_queue() already use, so a driver never disagrees
--     with itself about being closed across pages.
--   - p_from/p_to scope WHICH drivers are even considered (must have a last
--     active day inside the window, matching Overview's own date filter);
--     the streak itself still looks back through full history ending at that
--     last day, same convention as dds_driver_asset_weekly()/
--     dds_corrective_actions_queue() — a critical streak that started before
--     the filter window began doesn't reset just because the calendar filter
--     does.
-- ============================================================================
...
begin
  ...
  driver_last_day as (
    ...
    from (
      select emp_no as entity_id, shift_date from events
      where emp_no is not null and shift_date >= v_from and shift_date <= v_to
    ) x
    group by entity_id
  ),
