-- ============================================================================
-- DDS — 0113_corrective_actions_queue_date_cast_fix
-- ----------------------------------------------------------------------------
-- 0112 (itself a performance fix for 0111's timeout) failed outright on
-- first real call: `date - bigint` isn't a valid Postgres operator.
-- days_open comes from a CASE/count(*) expression, which Postgres types as
-- bigint even though its values are always small non-negative integers —
-- `oc.last_day - (oc.days_open - 1)` needs an explicit cast to make it
-- `date - integer`. Three occurrences, all in open_cases_totaled. No other
-- logic changed from 0112.
-- ============================================================================
