-- ============================================================================
-- DDS — 0115_dds_metrics_severity_trend
-- ----------------------------------------------------------------------------
-- Fixes a real, live bug found while scoping D-4 (sparkline data
-- availability): Overview's Sleep Alerts and Drowsiness Alerts KPI cards
-- (ov-kpi-sleep-spark / ov-kpi-drowsy-spark in index.html) show a correct
-- current value and delta (both computed from dds_metrics()'s whole-window
-- eventCodeDistribution via countByTier()), but their sparkline shape is
-- fed the TOTAL alerts per-day series (totalsSpark) — there was no
-- per-day severity breakdown in trend[] to draw a real one from, so both
-- cards silently plot the same line as the Total Alerts card, not their
-- own day-by-day shape.
--
-- Adds trend[].criticalUnits / trend[].highUnits, using the exact same
-- event_code ILIKE convention dds_alert_logs()'s p_severity filter already
-- uses (0101_dds_alert_logs_severity_filter.sql): sleep -> critical,
-- drowsiness-only -> high. No other field, CTE, or output shape changes.
-- ============================================================================
create or replace function public.dds_metrics(p_from date DEFAULT NULL::date, p_to date DEFAULT NULL::date, p_shift text DEFAULT NULL::text, p_asset_ids text[] DEFAULT NULL::text[], p_event_codes text[] DEFAULT NULL::text[], p_actionable_only boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
