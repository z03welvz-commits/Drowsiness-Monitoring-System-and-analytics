-- ============================================================================
-- DDS — 0161_dds_metrics_avg_alert_duration
-- ----------------------------------------------------------------------------
-- Adds avgAlertDurationSeconds to dds_metrics()'s kpis object, and
-- avgDurationSeconds to each trend[] day — the one real KPI the new
-- Analytics page (dashboard v1.1.5 layout) needs that this function didn't
-- already expose. Alert duration = end_time - start_time, the same
-- unambiguous elapsed-time definition used everywhere else this shape of
-- number appears in this app; avgSyncSeconds (end_time to update_time) is a
-- different measurement and is left untouched.
--
-- Purely additive: two new jsonb keys, no existing key's value or shape
-- changes, so every current caller (Analytics' own KPI row, its trend
-- sparklines) keeps working unchanged.
-- ============================================================================
...
begin
  ...
  kpis as (
    select
      ...
      avg(extract(epoch from (end_time - start_time)))
        filter (where end_time is not null and start_time is not null) as avg_duration_seconds,
  ...
  select jsonb_build_object(
    ...
    'kpis', jsonb_build_object(
      ...
      'avgAlertDurationSeconds', k.avg_duration_seconds,
      'actionableRatio',
