-- ============================================================================
-- DDS — 0114_corrective_actions_queue_recency_filter
-- ----------------------------------------------------------------------------
-- Live verification of 0113 surfaced a real usability issue, not a bug:
-- the top result by "Days Open" was flagged since July 4 with only 4 days
-- open — that driver's last recorded event was over two months ago, not
-- today. dds_driver_asset_weekly()'s status formula computes each entity's
-- streak relative to ITS OWN last event day, not current_date, so an
-- entity silent for months can still surface as "required" if their last
-- few active days happened to qualify.
--
-- That's correct for Driver & Asset Monitoring (a general status view), but
-- wrong for an "Open Cases, needs action today" queue. Per explicit
-- instruction: open_cases now also requires last_day >= current_date - 14,
-- so only entities with genuinely recent activity surface here. Status
-- computation itself (all_status) is untouched — an entity silent for
-- months still shows correctly elsewhere; it just doesn't appear in this
-- page's queue.
-- ============================================================================
...
begin
  ...
  open_cases as (
    select * from all_status where status = 'required' and last_day >= current_date - 14
  ),
  open_cases_totaled as (
    select oc.*,
      (oc.last_day - (oc.days_open - 1)::integer) as flagged_since,
      coalesce((
        select sum(dd.day_total) from driver_days dd
        where oc.entity_type = 'driver' and dd.entity_id = oc.entity_id
          and dd.shift_date >= (oc.last_day - (oc.days_open - 1)::integer)
      ), (
        select sum(ad.day_total) from asset_days ad
        where oc.entity_type = 'asset' and ad.entity_id = oc.entity_id
          and ad.shift_date >= (oc.last_day - (oc.days_open - 1)::integer)
      )) as event_count
    from open_cases oc
  ),
  closed_cases as (
