-- ============================================================================
-- DDS — 0140_driver_streaks_summary_counts
-- ----------------------------------------------------------------------------
-- UI audit: Driver Streaks is the only monitoring page with no KPI summary
-- row (Overview/Analytics/Driver & Asset Monitoring all open with one).
-- Adding one requires accurate counts across the FULL matching set, not just
-- whatever page of rows infinite-scroll has loaded client-side so far —
-- dds_driver_streaks() already computes v_total/v_open this way over
-- _driver_streak_rows, so this purely adds three more aggregates the same
-- way, no changes to any existing field, filter, or threshold:
--   - criticalCount: streak_days >= 2, the exact same rule already used
--     client-side by streakSeverity() to render the Critical/High badge
--   - monitoringCount / closedCount: mirrors the same status buckets
--     already computed per-row (status = 'monitoring'/'closed')
-- ============================================================================
