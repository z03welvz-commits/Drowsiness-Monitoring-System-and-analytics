-- ============================================================================
-- DDS — 0134_driver_streaks_deterministic_pagination
-- ----------------------------------------------------------------------------
-- Audit finding: dds_driver_streaks()'s final ORDER BY (whatever sort the
-- caller asked for, then streak_days desc, run_end desc as tiebreakers) has
-- no fully unique final column. Two drivers tied on all three — extremely
-- common: e.g. two different drivers both on a first-time 1-day streak that
-- ended on the same date, sharing the same status_priority (the default
-- sort) too — have no guaranteed stable relative order between separate
-- query executions. Postgres does not promise a repeatable order among tied
-- rows unless the ORDER BY is fully deterministic.
--
-- Infinite scroll fetches subsequent pages via increasing OFFSET across
-- SEPARATE queries (page 1: limit 50 offset 0, page 2: limit 50 offset 50,
-- ...). If Postgres orders a tied pair differently between those two
-- queries — a real possibility, not merely a theoretical one, since the
-- rows come from a freshly-built temp table each call — one of two things
-- happens: a row that should have been on page 1 lands on page 2 instead
-- (a different row appears twice, once on each page), or a row that
-- should have appeared on page 2 already appeared on page 1 and is now
-- skipped entirely. Both are real duplicate/missing-row bugs with a
-- completely different root cause than the client-side race already fixed
-- (that one only ever showed the SAME row twice from ONE request; this one
-- can show a DIFFERENT row twice, or drop one, across a scroll session).
--
-- Fix: append emp_no (unique per row in this result set) as the final
-- ORDER BY column, making the sort fully deterministic regardless of how
-- many rows tie on every other column.
-- ============================================================================
...
begin
  ...
  execute format(
    'select jsonb_agg(jsonb_build_object(
     ...
     -- caller actually asked to sort by; emp_no asc is the FINAL tiebreak,
     -- making this fully deterministic regardless of sort/status/date ties
     -- — required for offset-based pagination across separate queries
     -- (infinite scroll) to never skip or repeat a row.
