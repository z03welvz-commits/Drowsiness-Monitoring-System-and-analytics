-- ============================================================================
-- DDS — 0120_driver_streaks_date_filter
-- ----------------------------------------------------------------------------
-- The Driver Streaks table's from/to date filter was applied CLIENT-SIDE, on
-- top of a single server-paginated page (dds_driver_streaks() sorted by
-- streak_days desc, limit/offset). That's fine for the "Flags by Day" chart,
-- which pages through every matching row (up to its own 500-row cap) before
-- filtering client-side — but the table only ever fetches one page, so any
-- streak whose date range overlaps the chosen filter but which happens to
-- sort past that page (e.g. every single-day "High" streak, when dozens of
-- longer streaks outrank it under a streak_days-desc sort) was silently
-- dropped: never fetched, so the client-side filter never saw it. Reported
-- live as the chart's "Flags by Day" tooltip claiming 9 flags on a day while
-- the table showed only 2 rows for that same day.
--
-- Fix: filter by date range in SQL, before pagination — same overlap
-- semantics the client used (a streak's [run_start, run_end] intersects
-- [p_from, p_to]), so total/openCount and the sort/limit/offset window are
-- all computed against the correctly-filtered set. p_from/p_to default null
-- (no filter), so every existing caller keeps working unchanged.
-- ============================================================================
