-- ============================================================================
-- DDS — 0123_driver_streaks_staleness_fix
-- ----------------------------------------------------------------------------
-- 0122 dropped dds_driver_streaks()'s stale run_len>=3 gate so any qualifying
-- streak defaults to 'required' unless entity_status overrides it — correct
-- for CURRENT streaks, but latest_run picks the most recent qualifying run
-- in the entity's ENTIRE history with no recency bound, so a single bad day
-- from months ago now shows "Action Required" forever. Verified live: of
-- 315 rows, only 6 had their run_end in the last 7 days; 157 (half) were
-- over 90 days stale.
--
-- Fix, matching how dds_corrective_actions_queue()/dds_driver_asset_weekly()
-- already define "current" (anchored to the entity's own last active day,
-- not a fixed calendar window — correctly handles drivers who simply
-- weren't driving for a stretch rather than treating that as "resolved"):
-- a streak stays 'required' only while it IS the driver's most recent
-- activity (run_end equals their last event day anywhere in history — by
-- construction run_end can never exceed it, and if it's strictly less, the
-- days since were all non-qualifying, i.e. they've since had clean driving
-- and the flag is stale). Older ones fall back to 'resolved' — the row
-- stays visible (this page is explicitly "full history, not just recent"),
-- just no longer reads as an urgent open item. openCount (the sidebar
-- badge) is unaffected in shape — it already only counts 'required' rows —
-- but will now correctly shrink to genuinely current flags.
-- ============================================================================
...
begin
  ...
  select
    ...
    case
      ...
      when lr.run_end >= coalesce(dla.last_activity, lr.run_end) then 'required'
      else 'resolved'
    end as status,
    es.monitor_until,
