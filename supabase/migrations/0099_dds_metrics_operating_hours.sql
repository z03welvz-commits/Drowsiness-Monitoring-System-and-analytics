-- ============================================================================
-- DDS — 0099_dds_metrics_operating_hours
-- ----------------------------------------------------------------------------
-- Adds trend[].operatingHours — total MineStat operating hours per day,
-- summed across both shifts — so Analytics' "Alert Trend" chart can plot
-- Total Operating Hours instead of Distinct Assets (per direct instruction).
--
-- Source: public.minestat_shifts.operating_hrs (0024_minestat.sql) — a
-- SEPARATE ingest path from the DDS alert events this function otherwise
-- reads exclusively. minestat_shifts has its own (asset_id, shift_date,
-- shift) grain, so it's summed independently by shift_date and LEFT JOINed
-- onto the existing per-day trend rows, rather than folded into `filtered`
-- (which drives every OTHER field this function returns — kpis, hourly,
-- shiftDistribution, etc. — and must keep meaning "DDS alert events only").
-- A day with alerts but no MineStat data for the period (or vice versa)
-- gets 0, not a dropped row, on either side.
--
-- Filtered the same way `filtered` is (p_from/p_to/p_shift/p_asset_ids) so
-- the two series stay comparable — p_event_codes and p_actionable_only have
-- no MineStat equivalent and don't apply.
--
-- This is purely additive: every existing field's computation is byte-for-
-- byte unchanged. test/compare-derived.mjs and test/parity.sh both strip
-- trend[].operatingHours before diffing against derive() — derive()'s only
-- input is DDS alert-event rows, so it has no way to compute a MineStat-
-- sourced field and was never expected to.
-- ============================================================================
