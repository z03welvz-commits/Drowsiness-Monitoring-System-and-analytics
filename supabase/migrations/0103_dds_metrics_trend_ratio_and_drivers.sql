-- Analytics' Actionable Ratio and Drivers Involved KPI sparklines have
-- always rendered as a flat "no data" line (reported live: "its dead").
-- Root cause, confirmed by reading this function's own live definition:
-- trend[] only ever carried a per-day units/events/assets/avgSyncSeconds
-- shape — actionableRatio and distinctOperators were computed ONLY as
-- whole-window totals in `kpis`, with no per-day breakdown for the
-- sparkline to plot, so index.html's renderKpis() always passed null for
-- those two series and fell back to the flat baseline.
--
-- Adds actionable_units/distinct_operators to the existing `trend` CTE and
-- two new fields to each trend[] entry: `actionableRatio` (same formula as
-- kpis.actionableRatio, per day) and `distinctOperators` (count of distinct
-- emp_no per day, same as kpis.distinctOperators, per day). Purely
-- additive — every existing field on trend[] is untouched.
