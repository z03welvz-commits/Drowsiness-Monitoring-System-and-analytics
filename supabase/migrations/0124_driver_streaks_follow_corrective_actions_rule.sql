-- ============================================================================
-- DDS — 0124_driver_streaks_follow_corrective_actions_rule
-- ----------------------------------------------------------------------------
-- Live verification found dds_driver_streaks()'s (0123) "required" test and
-- dds_corrective_actions_queue()'s had drifted onto genuinely different
-- rules: of the drivers either page called required/open, only 9 of ~90
-- agreed (68 required in Driver Streaks vs 31 open in Corrective Actions,
-- drivers only). Same failure mode as #38 (Overview vs Driver Streaks), now
-- between a different pair of pages.
--
-- dds_driver_streaks() (0123) flags 'required' for ANY qualifying day (total
-- >= 20) that happens to be the driver's single most recent active day —
-- effectively a 1-day-minimum bar. dds_corrective_actions_queue() has always
-- required a longer bar: a 3+ consecutive-day trailing streak at >=20, OR a
-- single day > 10 (a lower total, but still a distinct rule).
--
-- Per direct instruction: Corrective Actions' rule is the one to keep —
-- Driver Streaks now adopts it instead. Implemented directly from data this
-- function already computes (latest_run + driver_last_activity), rather than
-- re-deriving Corrective Actions' own separate calendar-spine/89-day-window
-- mechanism: the trailing streak length ending on the driver's actual last
-- active day is latest_run.run_len exactly when that run's end equals the
-- driver's last_activity (the same equality 0123 already used to decide
-- "is this still current" — see driver_last_activity join below), 0
-- otherwise; the last active day's own total comes from one more join back
-- into `daily`. This changes only the required/resolved gate — Streak
-- Days/Streak Count and everything else on this page keep meaning exactly
-- what they did before (full history, not windowed), since driver_streaks()
-- deliberately doesn't apply Corrective Actions' own separate 14-day
-- "needs action now" recency filter (that stays specific to Corrective
-- Actions' own queue, not part of what "required" means here).
-- ============================================================================
...
begin
  ...
  select
    ...
    -- an active monitoring window=1, actioned=2, resolved (including an
    -- expired monitoring window)=3.
    case
      when es.status = 'monitoring' then
        case when es.monitor_until is not null and es.monitor_until >= current_date
             then 1 else 3 end
