-- ============================================================================
-- DDS — 0157_spike_investigation
-- ----------------------------------------------------------------------------
-- Part 4 of the action-logging/status redesign, continued. Direct answer to
-- the manager-defensibility scenario the redesign was built around: "why did
-- alert volume spike on this date range — is it driver-specific or
-- fleet-wide, and what corrective action was taken?"
--
-- For a chosen date range, ranks contributing drivers and assets by event
-- volume (same event-code breakdown shape as dds_driver_event_summary()/
-- dds_asset_event_summary()), reports each one's LIVE current case status
-- and most recent logged action (entity_status/entity_action_log — the same
-- live-join pattern 0156 added to Alert Logs, not a copy), a per-entity
-- reviewed/total event count from alert_cases.status_value, and a top5Share
-- figure (top 5 drivers' share of total window volume) as the fleet-wide-
-- vs-concentrated signal. p_from/p_to are both required — unlike the app's
-- other summary RPCs, there's no sensible default trailing window for
-- "investigate this specific spike."
-- ============================================================================

create or replace function public.dds_spike_investigation(
  p_from date,
  p_to date
)
returns jsonb
